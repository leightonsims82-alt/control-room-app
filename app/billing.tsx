...
export default function BillingScreen() {
  ...
  const billingSummary = useMemo(
    () => [
      ...
      `Additional managers: ${activeManagers.length} × £${ADDITIONAL_MANAGER_PRICE.toFixed(2)}/month`,
      `Projected monthly total: £${projectedMonthlyTotal.toFixed(2)}`,
    ].join('\n'),
    [activeManagers.length, projectedMonthlyTotal, siteSetup.siteName],
  );
  ...
  return (
    <AppScreen>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Set the pricing message inside the app, keep photo evidence local for now, and make every added manager a paid seat.</Text>
      </View>
      ...
      <SectionCard title="Add managers to this project" subtitle="Each additional manager must have their own paid seat before full project access is activated.">
      ...
      <SectionCard title="Billing summary" subtitle="This is a calculation preview for the current project. Payment processing still needs to be connected later.">
...
const styles = StyleSheet.create({
  ...
  summaryTextBox: { minHeight: 110, borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 14, padding: 12, backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: 'monospace', fontSize: 12, textAlignVertical: 'top' },
});
