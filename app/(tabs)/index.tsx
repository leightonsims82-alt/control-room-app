...
const mainCards: ProgrammeCard[] = [
  {
    ...
    subtitle: '23-week plot programme, stages and handover milestones',
  ...
  },
  {
    ...
    subtitle: 'Daily plot-by-plot progress in the Excel-style layout',
    icon: 'business-outline',
    colour: '#0f766e',
    href: '/plots',
  },
  {
    title: 'Trade Programmes',
    subtitle: 'Trade call-offs, lookaheads and recovery actions',
  ...
  {
    title: 'QA / Plot Story',
    subtitle: 'Photos, QA comments and plot progress evidence',
  ...
  {
    title: 'Exports',
    subtitle: 'Create site-ready Excel, PDF and wall chart outputs',
  ...
  return (
    <AppScreen>
      ...
      <View style={styles.summaryGrid}>
        <SummaryTile label="Active plots" value={sitePlots.length || 26} icon="business-outline" />
        <SummaryTile label="Trades this week" value={8} icon="people-outline" />
        <SummaryTile label="QA items" value={14} icon="shield-checkmark-outline" />
        <SummaryTile label="Exports ready" value={3} icon="download-outline" />
      </View>
        ...
        {mainCards.map((card) => (
          <Link key={card.title} href={card.href} asChild>
            <Pressable style={({ pressed }) => [styles.programmeCard, pressed && styles.pressed]}>
              <View style={[styles.cardIconWrap, { backgroundColor: `${card.colour}14` }]}>
