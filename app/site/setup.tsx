    ...
    <AppScreen>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.formGrid}>
          <Field label="Site name"><TextInput value={siteSetup.siteName} onChangeText={(siteName) => saveSiteSetup({ siteName })} placeholder="Site name" style={styles.input} /></Field>
          ...
          <Field label="Working week"><TextInput value={siteSetup.workingWeek} onChangeText={(workingWeek) => saveSiteSetup({ workingWeek })} placeholder="Monday to Friday" style={styles.input} /></Field>
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}><View style={styles.templateChips}>{plotTemplates.map((template) => { const active = template.id === selectedTemplateId; return <Pressable key={template.id} style={[styles.templateChip, active ? styles.templateChipActive : null]} onPress={() => { markChanged(); setSelectedTemplateId(template.id); }}><Text style={[styles.templateChipText, active ? styles.templateChipTextActive : null]}>{template.name}</Text></Pressable>; })}</View></ScrollView>

        {selectedTemplate ? (
          <View style={styles.templatePanel}>
            <View style={styles.formGrid}>
              <Field label="Template name"><TextInput value={selectedTemplate.name} onChangeText={(name) => savePlotTemplate({ ...selectedTemplate, name })} style={styles.input} /></Field>
              <Field label="Target weeks"><TextInput value={String(selectedTemplate.programmeWeeks)} onChangeText={(value) => savePlotTemplate({ ...selectedTemplate, programmeWeeks: Number(value) || 0 })} keyboardType="number-pad" style={styles.input} /></Field>
              <Field label="Stage count"><TextInput value={String(selectedTemplate.stageCount)} onChangeText={(value) => savePlotTemplate({ ...selectedTemplate, stageCount: Number(value) || 0 })} keyboardType="number-pad" style={styles.input} /></Field>
